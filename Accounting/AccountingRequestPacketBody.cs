...
namespace TacacsSharp.Accounting
{
    public class AccountingRequestPacketBody
    {
        ...
        public byte ArgCnt => (byte)(Args?.Length ?? 0);
        ...
        public byte[] ToArray()
        {
            using (var buffer = new MemoryStream())
            {
                using (var bw = new BinaryWriter(buffer))
                {
                    ...
                    bw.Write(RemAddrLen);
                    bw.Write(ArgCnt);
                    if(Args != null) foreach (var arg in Args) bw.Write((byte)arg.Length);
                    ...
                    if(Args != null) foreach (var arg in Args) bw.Write(Encoding.ASCII.GetBytes(arg));
                }
